...
##### Additional Observations on MultiGPUs
The way the multi-gpu is handled is not effective nor efficient, resulting into ***single GPU usage***. Previously, [nn.DataParallel]() was used, but looking at the documentation, it doesn't do the work.
We changed code in [base_model.py](../models/base_model.py#160) as it was confusing. 
#### Visualization
